...
counts = np.load(f"../counts/ammonia_LUCJ_L4_cc-pVDZ_zeroes.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/ammonia_LUCJ_L4_cc-pVDZ_zeroes.npz") 

EnergyPath = f"../energies/ammonia_LUCJ_L4_cc-pVDZ_zeroes.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "cc-pVDZ", "Molecule": "ammonia", "Method": f"LUCJ(L=4)/zeroes", "Energy": new_energy}
