...
counts = np.load(f"../counts/methane_LUCJ_L4_STO-3G_CCSD.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/methane_LUCJ_L4_STO-3G_CCSD.npz") 

EnergyPath = f"../energies/methane_LUCJ_L4_STO-3G_CCSD.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "STO-3G", "Molecule": "methane", "Method": f"LUCJ(L=4)/CCSD", "Energy": new_energy}
