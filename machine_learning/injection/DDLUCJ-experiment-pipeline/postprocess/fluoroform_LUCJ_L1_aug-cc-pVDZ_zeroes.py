...
counts = np.load(f"../counts/fluoroform_LUCJ_L1_aug-cc-pVDZ_zeroes.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/fluoroform_LUCJ_L1_aug-cc-pVDZ_zeroes.npz") 

EnergyPath = f"../energies/fluoroform_LUCJ_L1_aug-cc-pVDZ_zeroes.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "aug-cc-pVDZ", "Molecule": "fluoroform", "Method": f"LUCJ(L=1)/zeroes", "Energy": new_energy}
