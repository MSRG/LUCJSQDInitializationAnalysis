...
counts = np.load(f"../counts/ethylene_LUCJ_L2_aug-cc-pVDZ_random.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/ethylene_LUCJ_L2_aug-cc-pVDZ_random.npz") 

EnergyPath = f"../energies/ethylene_LUCJ_L2_aug-cc-pVDZ_random.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "aug-cc-pVDZ", "Molecule": "ethylene", "Method": f"LUCJ(L=2)/random", "Energy": new_energy}
