...
counts = np.load(f"../counts/formaldehyde_LUCJ_L3_STO-3G_MP2.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/formaldehyde_LUCJ_L3_STO-3G_MP2.npz") 

EnergyPath = f"../energies/formaldehyde_LUCJ_L3_STO-3G_MP2.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "STO-3G", "Molecule": "formaldehyde", "Method": f"LUCJ(L=3)/MP2", "Energy": new_energy}
