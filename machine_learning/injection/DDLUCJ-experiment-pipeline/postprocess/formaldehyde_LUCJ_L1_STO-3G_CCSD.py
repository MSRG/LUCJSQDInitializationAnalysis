...
counts = np.load(f"../counts/formaldehyde_LUCJ_L1_STO-3G_CCSD.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/formaldehyde_LUCJ_L1_STO-3G_CCSD.npz") 

EnergyPath = f"../energies/formaldehyde_LUCJ_L1_STO-3G_CCSD.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "STO-3G", "Molecule": "formaldehyde", "Method": f"LUCJ(L=1)/CCSD", "Energy": new_energy}
