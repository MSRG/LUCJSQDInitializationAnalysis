...
counts = np.load(f"../counts/formaldehyde_LUCJ_L5_STO-3G_ML.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/formaldehyde_LUCJ_L5_STO-3G_ML.npz") 

EnergyPath = f"../energies/formaldehyde_LUCJ_L5_STO-3G_ML.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "STO-3G", "Molecule": "formaldehyde", "Method": f"LUCJ(L=5)/ML", "Energy": new_energy}
