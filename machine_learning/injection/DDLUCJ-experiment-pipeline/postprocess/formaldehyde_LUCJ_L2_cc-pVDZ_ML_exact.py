...
counts = np.load(f"../counts/formaldehyde_LUCJ_L2_cc-pVDZ_ML_exact.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/formaldehyde_LUCJ_L2_cc-pVDZ_ML_exact.npz") 

EnergyPath = f"../energies/formaldehyde_LUCJ_L2_cc-pVDZ_ML_exact.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "cc-pVDZ", "Molecule": "formaldehyde", "Method": f"LUCJ(L=2)/ML_exact", "Energy": new_energy}
