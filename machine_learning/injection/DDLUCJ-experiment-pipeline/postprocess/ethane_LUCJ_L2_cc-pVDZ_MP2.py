...
counts = np.load(f"../counts/ethane_LUCJ_L2_cc-pVDZ_MP2.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/ethane_LUCJ_L2_cc-pVDZ_MP2.npz") 

EnergyPath = f"../energies/ethane_LUCJ_L2_cc-pVDZ_MP2.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "cc-pVDZ", "Molecule": "ethane", "Method": f"LUCJ(L=2)/MP2", "Energy": new_energy}
