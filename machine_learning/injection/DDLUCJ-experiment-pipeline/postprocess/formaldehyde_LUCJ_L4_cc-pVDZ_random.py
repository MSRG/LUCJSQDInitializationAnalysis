...
counts = np.load(f"../counts/formaldehyde_LUCJ_L4_cc-pVDZ_random.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/formaldehyde_LUCJ_L4_cc-pVDZ_random.npz") 

EnergyPath = f"../energies/formaldehyde_LUCJ_L4_cc-pVDZ_random.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "cc-pVDZ", "Molecule": "formaldehyde", "Method": f"LUCJ(L=4)/random", "Energy": new_energy}
