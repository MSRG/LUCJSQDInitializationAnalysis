...
counts = np.load(f"../counts/ethane_LUCJ_L1_cc-pVDZ_CCSD.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/ethane_LUCJ_L1_cc-pVDZ_CCSD.npz") 

EnergyPath = f"../energies/ethane_LUCJ_L1_cc-pVDZ_CCSD.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "cc-pVDZ", "Molecule": "ethane", "Method": f"LUCJ(L=1)/CCSD", "Energy": new_energy}
