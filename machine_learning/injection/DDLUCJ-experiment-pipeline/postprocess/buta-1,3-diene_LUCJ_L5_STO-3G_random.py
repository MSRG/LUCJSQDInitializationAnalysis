...
counts = np.load(f"../counts/buta-1,3-diene_LUCJ_L5_STO-3G_random.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/buta-1,3-diene_LUCJ_L5_STO-3G_random.npz") 

EnergyPath = f"../energies/buta-1,3-diene_LUCJ_L5_STO-3G_random.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "STO-3G", "Molecule": "buta-1,3-diene", "Method": f"LUCJ(L=5)/random", "Energy": new_energy}
