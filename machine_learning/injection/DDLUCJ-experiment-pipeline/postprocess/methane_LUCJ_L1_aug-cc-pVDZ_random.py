...
counts = np.load(f"../counts/methane_LUCJ_L1_aug-cc-pVDZ_random.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/methane_LUCJ_L1_aug-cc-pVDZ_random.npz") 

EnergyPath = f"../energies/methane_LUCJ_L1_aug-cc-pVDZ_random.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "aug-cc-pVDZ", "Molecule": "methane", "Method": f"LUCJ(L=1)/random", "Energy": new_energy}
