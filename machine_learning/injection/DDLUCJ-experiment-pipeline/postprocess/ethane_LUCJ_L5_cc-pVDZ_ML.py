...
counts = np.load(f"../counts/ethane_LUCJ_L5_cc-pVDZ_ML.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/ethane_LUCJ_L5_cc-pVDZ_ML.npz") 

EnergyPath = f"../energies/ethane_LUCJ_L5_cc-pVDZ_ML.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "cc-pVDZ", "Molecule": "ethane", "Method": f"LUCJ(L=5)/ML", "Energy": new_energy}
