...
counts = np.load(f"../counts/ammonia_LUCJ_L2_aug-cc-pVDZ_CCSD.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/ammonia_LUCJ_L2_aug-cc-pVDZ_CCSD.npz") 

EnergyPath = f"../energies/ammonia_LUCJ_L2_aug-cc-pVDZ_CCSD.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "aug-cc-pVDZ", "Molecule": "ammonia", "Method": f"LUCJ(L=2)/CCSD", "Energy": new_energy}
