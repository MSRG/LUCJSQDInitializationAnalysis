...
counts = np.load(f"../counts/ammonia_LUCJ_L4_aug-cc-pVDZ_ML.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/ammonia_LUCJ_L4_aug-cc-pVDZ_ML.npz") 

EnergyPath = f"../energies/ammonia_LUCJ_L4_aug-cc-pVDZ_ML.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "aug-cc-pVDZ", "Molecule": "ammonia", "Method": f"LUCJ(L=4)/ML", "Energy": new_energy}
