...
counts = np.load(f"../counts/fluoroform_LUCJ_L3_aug-cc-pVDZ_CCSD.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/fluoroform_LUCJ_L3_aug-cc-pVDZ_CCSD.npz") 

EnergyPath = f"../energies/fluoroform_LUCJ_L3_aug-cc-pVDZ_CCSD.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "aug-cc-pVDZ", "Molecule": "fluoroform", "Method": f"LUCJ(L=3)/CCSD", "Energy": new_energy}
