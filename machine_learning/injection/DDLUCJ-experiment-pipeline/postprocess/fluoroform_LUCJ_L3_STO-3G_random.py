...
counts = np.load(f"../counts/fluoroform_LUCJ_L3_STO-3G_random.npz")
bitstrings = counts['bitstrings']
bitstrings = BitArray.from_bool_array(bitstrings)

new_energy, subspace = initDDLUCJ(postprocess=True,BitArray=bitstrings,usefulqrum=True,bitarraypath=f"../counts/fluoroform_LUCJ_L3_STO-3G_random.npz") 

EnergyPath = f"../energies/fluoroform_LUCJ_L3_STO-3G_random.txt" 

with open(EnergyPath,'w') as f:
    new_row={"Basis Set": "STO-3G", "Molecule": "fluoroform", "Method": f"LUCJ(L=3)/random", "Energy": new_energy}
